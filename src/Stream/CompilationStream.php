...
/**
 * CompilationStream is a string reader that splits it into tokens on the fly.
 * Tokens are split following a specific compilation environment rule.
 *
 * @author Dorian Savina <dorian.savina@gmail.com>
 */
final class CompilationStream
{
    ...
    public function __construct(string $expression, CompilationEnvironment $environment)
    {
    ...
    public function tryNext(int $flags, $delimiters = null): bool
    {
        ...
        if ($flags & Atom::PREFIX_OPERATOR
            && $this->tryRead($this->environment->getPrefixOperatorPattern(), $symbol, $pregId)) {
            $this->lastToken = new PrefixAtom($symbol, $position, $this->environment->getPrefixOperator($symbol, $pregId));
            return true;
        }
        if ($flags & Atom::INFIX_OPERATOR
            && $this->tryRead($this->environment->getInfixOperatorPattern(), $symbol, $pregId)) {
            $this->lastToken = new InfixAtom($symbol, $position, $this->environment->getInfixOperator($symbol, $pregId));
            return true;
        }
    ...
    {
        $blank = $this->environment->getBlankPattern();
